...
import { Trash2, CheckCircle2, XCircle, AlertCircle, Download, Search, FileText, Home } from 'lucide-react';
...
export default function ErrorLogs() {
  ...
  const getStatusBadgeVariant = (status: ErrorLog['status']): "default" | "secondary" | "destructive" | "outline" => {
    switch (status) {
      case 'new':
        return 'destructive';
      case 'investigating':
  ...
  const getSourceBadgeVariant = (source: ErrorLog['source']): "default" | "secondary" | "destructive" | "outline" => {
    switch (source) {
      case 'edge-function':
        return 'destructive';
      case 'database':
  ...
  const filteredErrors = errors.filter((error) => {
    ...
    const matchesSearch =
      ...
      error.game_code?.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  if (!isLoaded || isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center text-white bg-gradient-to-b from-purple-900/40 via-red-900/50 to-black/70">
                          ...
                          <CardContent className="space-y-4">
                            ...
                            {error.function_name && (
                              <div>
                                ...
                                <p className="text-white">{error.function_name}</p>
                              </div>
                            )}
                            {error.stack && (
                              <div>
